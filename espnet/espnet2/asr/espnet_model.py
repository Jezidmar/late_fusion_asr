...
from espnet.nets.pytorch_backend.transformer.label_smoothing_loss import (  # noqa: H301
    LabelSmoothingLoss,
    LabelSmoothingLossMoE,
    LabelSmoothingLossMoECombined,
)
...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        lang_token_id: int = -1,
        load_balance_loss_coef: float = 0,
        z_loss_coef: float = 0,
        combined_moe: bool = False,
    ):
        ...
        self.combined_moe = combined_moe
        #
        ...
        if self.use_transducer_decoder:
            ...
            if self.combined_moe is True:
                self.criterion_att = None
                self.criterion_att_moe = LabelSmoothingLossMoECombined(
                    size=vocab_size,
                    padding_idx=ignore_id,
                    smoothing=lsm_weight,
                    normalize_length=length_normalized_loss,
                    num_experts=self.decoder.num_experts,
                    load_balance_loss_coef=self.load_balance_loss_coef,
                    z_loss_coeff=self.z_loss_coef,
                )
            if self.combined_moe is False:
                if self.z_loss_coef or self.load_balance_loss_coef:
                    self.criterion_att = None
                    self.criterion_att_moe = LabelSmoothingLossMoE(
                        size=vocab_size,
                        padding_idx=ignore_id,
                        smoothing=lsm_weight,
                        normalize_length=length_normalized_loss,
    ...
    def forward(
        ...
        **kwargs,
        ...
        #  #I need this to remove custom added layers.
        # force_gatherable: to-device and to-tensor if scalar for DataParallel
        loss, stats, weight = force_gatherable((loss, stats, batch_size), loss.device)
        torch.cuda.empty_cache()  # It is still to empty cache
    ...
    def _calc_att_loss(
        ...
        ys_in_lens = ys_pad_lens + 1
        # 1. Forward decoder
        if self.criterion_att:
            decoder_out, _ = self.decoder(
                encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens
            )
        if self.criterion_att_moe:
            decoder_out, _, rout_logits = self.decoder(
                encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens
            )

        # self.criterion_att_moe_z       <--Figure out how to include this loss as well.
        # 2.a what if I have MoE and I need logits from router?
        if self.criterion_att:
            # loss_att = self.criterion_att_moe(decoder_out,rout_logits, ys_out_pad)
            loss_att = self.criterion_att(decoder_out, ys_out_pad)
            # We use alpha coefficient which should be used for load balancing_loss AND Z-loss. 1e-4 HARDCODED
        if self.criterion_att_moe:
            loss_att = self.criterion_att_moe(decoder_out, rout_logits, ys_out_pad)
            rout_logits = []
