...
from espnet2.layers.log_mel import Cqt, Gamma, LogMel, Mfcc, Bark,WaveletP_v2,WaveletP_v3,MODGD
...
class DecodeEnseble:
    ...
    def _preprocess_loaded_audio(self, wave,speech_lengths, extractors,global_mvn_stats_files):
        ...
        for key, ext in extractors.items():
            ...
            #feats, feat_lengths = extractors[key](wave, None)
            #if key=="modgd":
            #    print(feats.size())
            #intermediate_out1 = global_mvn_stats_files[key](feats,feat_lengths)
            #print(features[key][1])


            features[key] = global_mvn_stats_files[key](feats)
            #import sys; sys.exit(0);feats_normalized, feat_lengths
            #features[key] = (feats_normalized, feat_lengths)
            features[key] = (features[key][0],torch.tensor([int(elem) for elem in features[key][1]]))
        return features

    def _recognize(
        self, model_dict, asr_train_args, beam_search, wave,speech_lengths, device, extractors,global_mvn_stats_files
    ):
        feats_dict = self._preprocess_loaded_audio(wave,speech_lengths, extractors,global_mvn_stats_files)
    ...
    def decode(self):
        ...
        if getattr(weights_conf, "decoder_mfcc") is not None:
            ...
            global_mvn_stats_files['mfcc']= GlobalMVN(stats_file='stats/mfcc_stats.npz' )
        ...
        if getattr(weights_conf, "decoder_gamma") is not None:
            ...
            global_mvn_stats_files['gamma']= GlobalMVN(stats_file='stats/gamma_stats.npz' )
        if getattr(weights_conf, "decoder_bark") is not None:
            ...
            global_mvn_stats_files['bark']= GlobalMVN(stats_file='stats/bark_stats.npz' )
        if getattr(weights_conf, "decoder_daubechies") is not None:
            ...
            extractors["daubechies"] = WaveletP_v2(
                fs=16000
            )
            global_mvn_stats_files['daubechies']= GlobalMVN(stats_file='stats/daubechies_stats.npz' )
        if getattr(weights_conf, "decoder_simlet") is not None:
            ...
            extractors["simlet"] = WaveletP_v3(
                fs=16000
            )
            global_mvn_stats_files['simlet']= GlobalMVN(stats_file='stats/simlet_stats.npz' )
        if getattr(weights_conf, "decoder_modgd") is not None:
            ...
            extractors["modgd"] = MODGD(
                fs=16000,
                window_size=400,
                hop_length=160,
            )
            global_mvn_stats_files['modgd']= GlobalMVN(stats_file='stats/modgd_stats.npz' )
        if getattr(weights_conf, "decoder_def_mel") is not None:
            ...
            global_mvn_stats_files['def_mel']= GlobalMVN(stats_file='stats/def_mel_stats.npz' )
        nbest = 1
            ...
            for keys, batch in loader:
                ...
                with torch.no_grad():
                    results = self._recognize(
                        model_dict, asr_train_args, beam_search, wave,speech_lengths, conf.device, extractors, global_mvn_stats_files
                    )
                if conf.device== "cuda":
...
if __name__=="__main__":
    ...
    parser.add_argument("--data_path_and_name_and_type", type=str2triple_str,action="append")
