...
class EnsembleBeamSearch(torch.nn.Module):
    ...
    def search(
        ...
        device = "cpu"
        best_hyps = []
        calibrated_weights = torch.Tensor([1.0117, 1.0117, 1.0117, 1.0117])
        ...
        for hyp in running_hyps:
            ...
            for i, k in enumerate(self.full_scorers):
                # Calibrated_weights , device='cuda:0')
                weighted_scores += self.weights[k] * scores[k] * calibrated_weights[i]

            part_ids = torch.topk(weighted_scores, self.pre_beam_size)[
                1
            ]  # Select only the 1.5 * beam_size ids
