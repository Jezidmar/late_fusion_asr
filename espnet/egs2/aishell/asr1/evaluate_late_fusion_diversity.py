...
from modified_beam_search_diversity import EnsembleBeamSearchDiversity
...
class DecodeEnseble:
    ...
    def _recognize(
        ...
        hyp, Elems = beam_search(x=encoder_outs, maxlenratio=0.6, minlenratio=0.0)
        ...
        # remove blank symbol id, which is assumed to be
        logging.info(f"Token int before removing 0: {token_int}")
        token_int = list(filter(lambda x: x != 0, token_int))
        logging.info(f"Token int after removing 0: {token_int}")
    ...
    def find_transcript(self, key, ref, tokenizer, converter):
        # Go through all text files.
        transcription = ""
        for elem in ref:
            key_search = elem.split(" ")[0]
            text = elem.split(" ")[1]
            if key in key_search:
                transcription = text
                break
        logging.info(f"Transcription: {transcription}")
        # Now perform the tokenization
        tokens = tokenizer.text2tokens(transcription)
    ...
    def decode(self):
        ...
        if getattr(weights_conf, "decoder_def_mel") is not None:
            ...
            extractors["def_mel"] = DefaultFrontendDEF(
                n_fft=512, hop_length=128, fs=16000
            )
        ...
        converter = TokenIDConverter(token_list=asr_train_args.token_list)
        with open("dump/raw/test/text", "r") as f:
            ref = f.readlines()
        logging.info(f"Number of lines in the ground truth transcript is: {len(ref)}")

        All_outputs = []
        with DatadirWriter(output_dir) as writer:
            for keys, batch in loader:
                logger.info(f"Key is :{keys} and batch is : {batch}")
                ...
                key = keys[
                    0
                ]  # This is the utterance key now I have to find the corresponding transcript...

                transcript_in_tokens = self.find_transcript(
                    key, ref, tokenizer, converter
                )
                transcript_in_tokens[-1] = 4232
                logging.info(f"Transcription in tokens: {transcript_in_tokens}")
                beam_search = EnsembleBeamSearchDiversity(
                    ...
                    transcript_in_tokens=transcript_in_tokens,
                )
                ...
                logging.info(f"Elems in eval file: {Elems}")
                Converted_data = [
                    [
                        int(item) if isinstance(item, torch.Tensor) else item
                        for item in sublist
                    ]
                    for sublist in Elems
                ]
                logging.info(f"Converted elems in eval file: {Converted_data}")
                All_outputs.append(Converted_data)
                # Normal ASR
                ...
                for n, (text, token, token_int, hyp) in zip(
                    ...
                    ibest_writer["diversity"][key] = " ".join(map(str, Converted_data))
                    if text is not None:
