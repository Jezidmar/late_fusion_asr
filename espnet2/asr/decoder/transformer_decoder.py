...
from espnet.nets.pytorch_backend.transformer.decoder_layer import DecoderLayer,DecoderLayerMOE,SMoE,SMoE_MHA,SMoE_combined
from espnet.nets.pytorch_backend.transformer.dynamic_conv import DynamicConvolution
from espnet.nets.pytorch_backend.transformer.dynamic_conv2d import DynamicConvolution2D
from espnet.nets.pytorch_backend.transformer.embedding import PositionalEncoding
from espnet.nets.pytorch_backend.transformer.layer_norm import LayerNorm,RMSNorm
...
class BaseTransformerDecoderMOE(AbsDecoder, BatchScorerInterface):
    """Base class of Transfomer decoder module.
    ...
    def forward(
        ...
        intermediate_outs = []
        for layer_idx, decoder_layer in enumerate(self.decoders):
            x, tgt_mask, memory, memory_mask,moe_out  = decoder_layer(
                x, tgt_mask, memory, memory_mask
            )
            if return_all_hs:
                intermediate_outs.append(x)
        ...
        olens = tgt_mask.sum(1)
        if return_hs:
            return (x, hidden), olens,moe_out
        elif return_all_hs:
            return (x, intermediate_outs), olens,moe_out
        return x, olens,moe_out
    ...
    def forward_one_step(
        ...
        new_cache = []
        for c, decoder in zip(cache, self.decoders):
            x, tgt_mask, memory, memory_mask,_ = decoder(  #I added '_' as return so as 
                x, tgt_mask, memory, None, cache=c
            )
            new_cache.append(x)
...
class TransformerDecoder_MOE_SPARSE(BaseTransformerDecoderMOE):
    def __init__(
        self,
        vocab_size: int,
        encoder_output_size: int,
        ...
        if use_rms_norm==True:    
            self.after_norm=RMSNorm()    

        attention_dim = encoder_output_size
        self.decoders = repeat(
            num_blocks,
            lambda lnum: DecoderLayerMOE(
                attention_dim,
                MultiHeadedAttention(
                    attention_heads, attention_dim, self_attention_dropout_rate
                ),
                ...
                SMoE(attention_dim, attention_dim*4, num_experts,top_k,activation_tr_block),
                dropout_rate,
                normalize_before,
                concat_after,
            ),
...
class TransformerDecoder_MOE_SPARSE_MH(BaseTransformerDecoderMOE):
    def __init__(
        self,
        vocab_size: int,
        encoder_output_size: int,
        ...
        if use_rms_norm==True:    
            self.after_norm=RMSNorm(dim=encoder_output_size)    
        attention_dim = encoder_output_size
        self.decoders = repeat(
            num_blocks,
            lambda lnum: DecoderLayerMOE(
                ...
                SMoE_MHA(attention_dim, attention_dim*4,num_experts,top_k, num_heads,activation_tr_block),
                dropout_rate,
                normalize_before,
                concat_after,
            ),
...
class TransformerDecoder_Combined_MOE(BaseTransformerDecoderMOE):
    def __init__(
        self,
        vocab_size: int,
        encoder_output_size: int,
        ...
        self.decoders = repeat(
            ...
            lambda lnum: DecoderLayerMOE(
                ...
                SMoE_combined(attention_dim, attention_dim*4,num_experts,top_k, num_heads,activation_tr_block),
                dropout_rate,
                normalize_before,
                concat_after,
            ),
